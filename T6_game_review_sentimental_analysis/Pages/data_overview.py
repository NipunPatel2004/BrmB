...
def data_overview_page():
    ...
    with col1:
        st.metric("Total Reviews", f"{total_rows:,}")
    ...
    st.dataframe(df.head(100), use_container_width=True, height=500, hide_index=True)
    ...
    if missing_df.empty:
        ...
        st.success("Excellent! No missing values were found.")
    ...
    if duplicate_rows == 0:
        ...
        st.warning(
            f" {duplicate_rows:,} duplicate records " "were found in the dataset."
        )
    ...
    if "review_score" in df.columns:
        ...
        st.info("The 'review_score' column was not found.")
    ...
    if "review_text" in df.columns:
        ...
        word_count = df["review_text"].astype(str).str.split().str.len()

        col1, col2, col3, col4 = st.columns(4)

        with col1:
            st.metric("Average Characters", f"{text_length.mean():.0f}")

        with col2:
            st.metric("Average Words", f"{word_count.mean():.1f}")

        with col3:
            st.metric("Shortest Review", f"{text_length.min():,} chars")

        with col4:
            st.metric("Longest Review", f"{text_length.max():,} chars")

    else:

        st.info("The 'review_text' column was not found.")
    ...
    if quality_score == 100:
        ...
        st.warning(f"Dataset Quality Score: **{quality_score}/100**")

    else:

        st.error(f"Dataset Quality Score: **{quality_score}/100**")
